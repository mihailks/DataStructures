...
public class RedBlackTree<T extends Comparable<T>> {
    ...
    public void insert(T value) {
        ...
        this.root.color = BLACK; // the root is always black
    }
    ...
    private Node<T> rotateRight(Node<T> node) {
        ...
        node.count = 1 + getNodesCount(node.left) + getNodesCount(node.right); //child element after rotation
...
//        result.count = node.count;
    ...
    public static class Node<T extends Comparable<T>> {
        ...
        public Node(T value) {
            this.value = value;
            this.color = RED; // normally it is every new node that we insert is red
            this.count = 1; // when is empty and we add the count is 1
        }
